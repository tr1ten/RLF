...
class ExpectedSarsaAgent():
    ...
    def agent_step(self, reward, observation):
        ...
        gaction = self.argmax(current_q)
        naction = list(range(self.num_actions))
        naction.remove(gaction)
        each_p = self.epsilon/self.num_actions;
        expected_term = ((1-self.epsilon+each_p)*self.q[state,gaction]+each_p*np.sum(self.q[state,naction]));
        # Perform an update
        # --------------------------
